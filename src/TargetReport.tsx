// AJR cibles issus du profil (onglet « Comptes rendus », partie haute).
//
// Énergie (BMR / TDEE / calories cibles), macronutriments (avec détail des acides
// aminés indispensables et des acides gras) et micronutriments (repliable). Ces
// sections proviennent d'App et n'ont subi AUCUN changement de calcul : le
// composant reçoit le `DailyReport` déjà calculé et se contente de l'afficher.
// Les infobulles éducatives (TIPS / MICRO_TIPS) sont du contenu UI, indépendant
// du calcul, indexé sur le nom exact de chaque ligne (cf. macros.ts / micros.ts).
import type { ReactNode } from "react";
import type { DailyReport } from "./calc/report";
...
export function TargetReport({ report }: { report: DailyReport }) {
  return (
    <>
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">Métabolisme de base</p>
          <p className="mt-1 text-2xl font-bold">{round(report.bmrKcal)} <span className="text-base font-normal text-slate-400">kcal/j</span></p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">Dépense totale (TDEE)</p>
          <p className="mt-1 text-2xl font-bold">{round(report.tdeeKcal)} <span className="text-base font-normal text-slate-400">kcal/j</span></p>
        </div>
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm">
          <p className="text-sm text-emerald-700">Calories cibles</p>
          <p className="mt-1 text-2xl font-bold text-emerald-800">{round(report.energyKcal)} <span className="text-base font-normal text-emerald-600">kcal/j</span></p>
          <p className="mt-1 text-xs text-emerald-700">
            {fmtAdjustment(report.energy.adjustmentKcal)} · {round(report.energy.energyMinKcal)}–{round(report.energy.energyMaxKcal)} kcal/j
          </p>
        </div>
      </section>

      {report.weightAdjusted && (
        <p className="mt-4 rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Les macros en g/kg sont calculées sur un poids de référence de{" "}
          {one(report.effectiveWeightKg)} kg — la moyenne entre votre poids
          actuel ({one(report.profile.weightKg)} kg) et votre poids cible
          ({one(report.profile.targetWeightKg)} kg).
        </p>
      )}
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-4">
          <MacroRow label="Protéines" bar="bg-sky-500" target={report.macros.protein}>
            <details className="group mt-2">
              ...
              <div className="mt-2 grid grid-cols-1 gap-x-8 gap-y-1 sm:grid-cols-2">
                {report.macros.aminoAcids.map((aa) => (
