...
export interface NutritionProfile {
  ...
  // Chaque objectif porte son propre niveau d'activité implicite (plus de
  // sélecteur « niveau d'activité » séparé).
...
export interface Profile {
  sex: Sex;
  ageYears: number;
  weightKg: number; // poids actuel
  targetWeightKg: number; // poids cible visé
  heightCm: number;
  goal: NutritionGoal;
}
...
export function validateProfile(p: Profile): void {
  ...
  if (!(p.targetWeightKg > 0 && p.targetWeightKg <= 400))
    throw new Error("Poids cible hors limites plausibles (0-400 kg).");
...
// Poids de référence pour les calculs en g/kg : moyenne entre le poids actuel et
// le poids cible. Doser les macros sur cette moyenne évite de surdoser sur le
// poids de départ (perte) ou de sous-doser sur un objectif encore lointain
// (prise de masse), tout en accompagnant la progression vers la cible.
export function effectiveWeightKg(p: Profile): number {
  return (p.weightKg + p.targetWeightKg) / 2;
}
